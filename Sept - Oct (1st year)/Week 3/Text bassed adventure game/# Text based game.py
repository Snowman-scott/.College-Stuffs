...
class SaveManager:
    ...
    def load_game_data(self, save_folder_name, filename=None):
        ...
        while True:
            ...
            if move == "current":
                ...
                if self.cl == pm:
                    ...
                    print("you have moved to" ,self.House[self.cl][pm])
                    time.sleep(1)
                    self.cl = self.House[self.cl][pm]
                else:
                    print("Invalid input")
    
            elif move == "pickup":
                if "Item" in self.House[self.cl]:
                    item = self.House[self.cl]["Item"]
                    if item not in self.inventory:
                        self.inventory.append(item)
                        print(f"You picked up: {item}")
                        del self.House[self.cl]["Item"]
                    else:
                        print("You already have this item!")
                else:
                    print("There are no items to pickup here")
                    time.sleep(1)
