...
def get_data_by_id_and_date(activity_id, start_date, end_date):
    ...
    activity_data["Date"] = pd.to_datetime(activity_data["Date"], format="%d/%m/%Y", errors="raise")

    date_range = (activity_data["Date"] >= pd.to_datetime(start_date, format="%d/%m/%Y")) &                  (activity_data["Date"] <= pd.to_datetime(end_date, format="%d/%m/%Y"))
...
def calculate_total_places_sold(date_id, activity_id, start_date, end_date):
    ...
    print('The total number of places sold for activity {}, between {} and {} was: {}'.format(activity_id, start_date, end_date, total_places))
